...
当前重点不是增加远程业务能力，而是让 `dev` 只验证、`main` 自动发布，且 `VERSION` 文件成为唯一默认版本来源。
...
`Makefile` 默认读取仓库根目录的 `VERSION`。发布新版本时，只更新 `VERSION`，推送到 `dev` 验证，通过后 merge 到 `main`。
...
`Dev Verification` workflow 会在 `dev` 和到 `main` 的 PR 上做完整验证，但不会创建 tag 或 GitHub Release。

`Main Release` workflow 会在代码进入 `main` 后读取 `VERSION`，完成测试、构建、打包、checksum 校验、创建 tag、创建 GitHub Release 并上传 release assets。

包含 `-rc`、`-beta`、`-alpha` 的版本会自动标记为 pre-release。正式版例如 `v0.1.0` 不会标记 pre-release。如果同名 tag 或 GitHub Release 已存在，workflow 会失败，不会自动覆盖；需要重发 RC 时请发布新版本，例如 `v0.1.0-rc2`。
...
分支发布流程见 [docs/branching-and-release.md](docs/branching-and-release.md)。
